...
class Rabi(Sequence):
    ...
    def qua_sequence(self):
        ...
        align()
        play('control'*amp(self.rabiAmp()), 'Q1', duration = self.tRabi())
        align()
        wait(self.tPostControl(),'Q1')
        align()
        #self.control.shotTime = seq.tRabi this should set a certain value after execution
    ...
    def get_default_seq_config(
        ...
        """
        config = {
            'tRabi': {'unit': 'cycles', 'value': int(0.1e3/4)},
            'tPostControl': {'unit': 'cycles', 'value': int(80e3/4)},
            'rabiAmp': {'unit': 'V', 'value': 0.9},

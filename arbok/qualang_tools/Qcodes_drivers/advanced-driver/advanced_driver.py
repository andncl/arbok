...
class OPXCustomSequence(OPX):
    ...
    This specific experiment consists of a custom pulse sequence (to be set with
    the self.pulse_sequence() attribute) embedded in an infinite loop with a
    pause statement at the beginning of each iteration to allow the
    synchronization with the qcodes do"nd" scans.

    The readout element, operation and duration can be parametrized, as well as 
    the acquisition_mode that can be 'raw_adc', 'full_integration',
    'full_demodulation', 'sliced_integration' or 'sliced_demodulation'.
    A specific mode for which the OPX will perform a measurement when it
    receives a trigger can be enabled by setting wait_for_trigger(True).

    The parameter sweeps performed by the OPX can also be parametrized using the
    opx_scan() attribute that can be '0d','1d' or '2d'. The corresponding scan
    axes are accessible via axis1_start(), axis1_stop() and axis1_step().
    ...
    def update_readout_parameters(self):
        """
        Update the config with the readout length and amplitude set with 
        self.readout_pulse_length() and self.readout_pulse_amplitude().
        """
    ...
    def measurement_declaration(self):
        """
        Declare the QUA variables for the measurement based on the 
        self.acquisition_mode() parameter.
        """
    ...
    def measurement(self):
        """
        Create the QUA program corresponding to the measurement mode based on
        the self.acquisition_mode() parameter.
        """
    ...
    def stream_results(self):
        """
        Create the stream processing based on the measurement mode
        (self.acquisition_mode()) and OPX scan 0d, 1d or 2d (self.opx_scan())
        """
        variables = self.measurement_variables
        if self.opx_scan() == "0d":
            if (
